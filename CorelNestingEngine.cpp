/* =====================================================================
   CorelNestingEngine.cpp   (v2.1)  -- correct bounding-box nester.
   ...
   NFP is Overlap()/DropY()/PlaceOnSheet() -> replace the rect overlap
   test with an NFP/Minkowski test (Deepnest's minkowski.cc) in Stage 2.
   ===================================================================== */
...
// fit rule: 0 Bottom (best) / 1 Width (best) -> lowest y first (fill the
// full sheet width, grow upward). 2 Height (best) -> lowest x first (fill
// the full sheet height, keep the used width minimal).
inline double FitScore(int fitMode, double x, double y) {
    return (fitMode == 2) ? (x * 1e6 + y) : (y * 1e6 + x);
}
...
bool PlaceOnSheet(const Engine* e, const std::vector<Rect>& placed,
    ...
    for (double x : xs) {
        ...
        const double score = FitScore(e->fitMode, x, y);
...
static int RunImpl(Engine* e) {
    ...
    for (int idx : order) {
        ...
        double bX = 0, bY = 0, bRot = 0, bW = 0, bH = 0;
        int    bSheet = -1;
        ...
        for (double ang : AngleSet(e)) {
            ...
            const double wPure = (x1 - x0), hPure = (y1 - y0);
            // "too big" = the part ITSELF exceeds the usable sheet. spacing
            // is enforced between parts only, never against the sheet edge
            // (edge distance = reduce + edgePadding), so exact-fit parts pass.
            if (wPure > usableW + EPS || hPure > usableH + EPS) continue;
            const double w = wPure + gap;        // collision footprint
            const double h = hPure + gap;
            const double limW = usableW + gap;   // lets the footprint's gap
            const double limH = usableH + gap;   //   hang past the edge
            ...
            for (int si = 0; si <= (int)e->sheets.size(); ++si) {
                ...
                double px, py;
                if (!PlaceOnSheet(e, *placed, w, h, limW, limH, px, py))
                    continue;
                // prefer earliest sheet, then fit score
                double score = si * 1e12 + FitScore(e->fitMode, px, py);
                if (score < bestScore) {
...
CN_EXPORT int CN_CALL CN_GetVersion(void) { return 210; }
